...
app.post("/api/gemini/mentor-advice", aiLimiter, async (req, res) => {
  try {
    ...
    bio = truncateString(bio, 1000);
    topic = truncateString(topic, 1000);
    skills = sanitizeAndTruncateArray(skills, 10, 50);
    interests = sanitizeAndTruncateArray(interests, 10, 50);

    const prompt = `
You are a warm, wise, encouraging, and experienced African female entrepreneur and business mentor (acting as a "BIG Sister" / "BIG Mentor").
You are advising Sarah Jenkins (an Aspiring Fashion Founder in Lagos) or another sister who has the following profile:
- Name: ${name || 'Sarah Jenkins'}
- Title: ${title || 'Aspiring Fashion Founder'}
- Location: ${city || 'Lagos'}
- Skills: ${skills.length ? skills.join(', ') : 'Apparel Design'}
- Interests: ${interests.length ? interests.join(', ') : 'Sustainable Fashion, E-Commerce'}
- Bio: ${bio || 'Fashion designer looking to scale my artisan-made brand.'}

She is asking for guidance on the following topic: "${topic || 'General business growth & scaling sustainably'}".

Provide a direct, inspiring, and highly actionable mentoring response. Use the tone of a supportive sister, but with professional, tactical business wisdom (referencing the local ecosystem like mobile money, cooperative funding, regional trade (AfCFTA), or local retail logistics where appropriate). Keep the response structured in 2-3 short, clear, and punchy paragraphs. Finish with a warm, encouraging closing signature.
`;
    ...
    console.error("Error in mentor-advice:", error);
    res.status(500).json({ error: "Failed to generate mentor advice due to an internal AI model error" });
  }
});
...
app.post("/api/gemini/pitch-builder", aiLimiter, async (req, res) => {
  try {
    ...
    // Strict Input Validation & Length Limits
    businessName = truncateString(businessName, 150);
    description = truncateString(description, 1500);
    targetAudience = truncateString(targetAudience, 1000);
    uniqueValue = truncateString(uniqueValue, 1000);

    const prompt = `
Act as a premium business development expert and female founder coach.
Create a compelling, professional 60-second elevator pitch and business outline based on this information:
- Business Name: "${businessName}"
- Description/Mission: "${description}"
- Target Audience: "${targetAudience}"
- Unique Value Proposition: "${uniqueValue}"

Provide a structured, beautifully formatted markdown response containing:
1. **The Hook** (An attention-grabbing opening question or statistic)
2. **The Problem & Solution** (Vividly illustrating the pain point and how ${businessName} solves it)
3. **The 60-Second Elevator Pitch** (A complete, fluent, and inspiring paragraph ready to memorize and recite)
4. **Strategic Growth Steps** (3 bulleted actionable next steps to launch or scale)

Make it sound highly professional, passionate, and tailored for investors, retail partners, or cooperative backers in the African market.
`;
    ...
    console.error("Error in pitch-builder:", error);
    res.status(500).json({ error: "Failed to generate business pitch due to an internal AI model error" });
  }
});
...
app.post("/api/gemini/confidence-coaching", aiLimiter, async (req, res) => {
  try {
    ...
    // Strict Input Validation & Length Limits
    journalText = truncateString(journalText, 2500);

    const prompt = `
You are a warm, nurturing, and powerful Confidence Coach for the "Be Independent Gal" (BIG) community, helping women overcome self-doubt, imposter syndrome, and navigate startup or career blockages.
A sister has written this reflection/blockage in her daily journal:
"${journalText}"

Respond with deep empathy, sisterly love, and inspiring words. Suggest 1 practical daily mindfulness exercise (e.g. 5-minute mirror technique, breathing cycle, or vocal confirmation) and 1 small immediate action she can take today to build her momentum. Keep it empowering, positive, and concise (under 250 words).
`;
    ...
    console.error("Error in confidence-coaching:", error);
    res.status(500).json({ error: "Failed to generate coaching response due to an internal AI model error" });
  }
});
